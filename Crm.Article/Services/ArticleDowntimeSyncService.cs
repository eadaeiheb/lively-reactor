namespace Crm.Article.Services
{
	...
	public class ArticleDowntimeSyncService : DefaultSyncService<ArticleDowntime, Guid>
	{
		private readonly IEnumerable<IContactSyncService> articleSyncServices;
		private readonly IAuthorizationManager authorizationManager;
		public ArticleDowntimeSyncService(IRepositoryWithTypedId<ArticleDowntime, Guid> repository, RestTypeProvider restTypeProvider, IRestSerializer restSerializer, IMapper mapper, IComponentContext context, IAuthorizationManager authorizationManager)
			: base(repository,
				restTypeProvider,
				restSerializer,
				mapper)
		...
		public override ArticleDowntime Save(ArticleDowntime entity)
		{
			return repository.SaveOrUpdate(entity);
		}
		public override IQueryable<ArticleDowntime> GetAll(User user, IDictionary<string, int?> groups, IDictionary<string, Guid> clientIds)
		{
			var entities = repository.GetAll();

			Expression<Func<ArticleDowntime, bool>> predicate = null;
			...
			return entities
				.Where(x => x.To >= DateTime.UtcNow.AddDays(-1));
		}
	}
}
