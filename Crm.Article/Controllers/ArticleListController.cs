namespace Crm.Article.Controllers
{
	...
	public class ArticleListController : GenericListController<Article>
	{
		...
		public class ArticleCsvDefinition : CsvDefinition<Article>
		{
			...
			public override IEnumerable<KeyValuePair<string, Func<Article, object>>> GetCsv(IEnumerable<Article> items)
			{
				...
				Property("QuantityUnit", x => x.QuantityUnitEntry != null ? qtyUnits.FirstOrDefault(c => c.Key == x.QuantityUnitEntry.QuantityUnitKey)?.Value : string.Empty);
				Property("Description", x => x.Description);

				//Internal Ids
				Property("ArticleTypeKey", x => x.ArticleTypeKey);
				Property("CurrencyKey", x => x.CurrencyKey);
				Property("QuantityUnitKey", x => x.QuantityUnitEntry?.QuantityUnitKey);
