namespace Crm.Order.Model
{
	...
	public abstract class BaseOrder : EntityBase<Guid>, ISoftDelete, IEntityWithVisibility, IExportable
	{
		...
		public virtual ICollection<OrderRecipient> OrderRecipients { get; set; }
		...
		public virtual ICollection<OrderCommunicationData> SendConfirmationTo
		{
			get
			{
				var orderCommunications = new List<OrderCommunicationData>();
				if (OrderRecipients.Count == 0)
				{
					orderCommunications.Add(new OrderCommunicationData
					{
						Data = CustomFax,
						Title = ContactPerson?.ToString() ?? String.Empty,
						Type = "fax"
					});
				}
				orderCommunications.AddRange(OrderRecipients.Select(recipient => new OrderCommunicationData
				{
					Data = recipient.Email,
					Title = recipient.ContactPersonId == ContactPersonId ? ContactPerson?.ToString() : String.Empty,
					Type = "email"
				}));
		...
		public BaseOrder()
		{
			...
			OrderRecipients = new List<OrderRecipient>();
			StatusKey = "Open";
