namespace Crm.Order.Model.Configuration
{
	...
	public class OrderConfiguration : EntityConfiguration<Order>
	{
		public override void Initialize()
		{
			...
			Property(x => x.ContactPersonId, c => c.Filterable(f => f.Definition(new AutoCompleterFilterDefinition<Person>("PersonAutocomplete", new { Plugin = "Crm" }, "Crm_Person", x => x.Name, x => x.Id, x => x.Surname, x => x.Firstname) { Caption = "ContactPerson" })));
			Property(x => x.Representative, c => c.Filterable(f => f.Definition(new UserFilterDefinition { WithGroups = true })));
			Property(x => x.ContactId, m => m.Filterable(f => f.Definition(new AutoCompleterFilterDefinition<Company>("CompanyAutocomplete", new { Plugin = "Crm" }, "Crm_Company", "Helper.Company.getDisplayName", x => x.Id, x => x.LegacyId, x => x.Name) { ShowOnMaterialTab = false })));
