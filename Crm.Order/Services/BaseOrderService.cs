namespace Crm.Order.Services
{
	...
	public class BaseOrderService : IBaseOrderService, IMerger<Contact>
	{
		...
		public virtual bool TrySendMail(BaseOrder order)
		{
			...
			foreach (var communicationData in orderCommunicationData)
			{
				var comData = communicationData;
				orderCommunicationDataTransformers.ForEach(transformer => comData = transformer.TransformData(order, comData));
				if (comData.Handled || comData.Data == null)
				{
					return false;
				}
				...
				{
					message.From = new MailboxAddress(userService.CurrentUser.DisplayName, userService.CurrentUser.Email).ToString();
				}
				...
				message.Recipients.Add(comData.Data);
				message.AttachmentIds.Add(fileService.CreateAndSaveFileResource(order, CreatePdf(order), MediaTypeNames.Application.Pdf, message.Subject + ".pdf").Id);
